...
import { CadastroMovimentacaoComponent } from 'src/app/forms/cadastro-movimentacao/cadastro-movimentacao.component';
...
export class MovimentacaoComponent implements OnInit {
  ...
  NovoRegistro(): void {
    const dialogRef = this.dialog.open(CadastroMovimentacaoComponent, {
      width: '800px',
      hasBackdrop: true,
      disableClose: true,
      data: { id: 0 }
        ...
        .then(result => {
          if (result.value) {
            this.selection.selected.forEach(item => {
              this.firestore.doc(`movimentacao/` + item).delete();
            });
          }
        });
    } else {
